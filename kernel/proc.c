...
int age[] = {1, 2, 4, 8, 16};
struct PriorityQueue queuetable[5];
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->trapframe->a0 = 0;
  // copy the same no. of tickets from parents to the child
  np->ticket = p->ticket;
...
void scheduler(void)
{
  ...
  while (1)
  {
    ...
    intr_on();
    int ticketstillnow = 0; // to compare which process is first.
    int totalTickets = 0;
    for (p = proc; p < &proc[NPROC]; p++)
    {
      if (p->state == RUNNABLE)
        totalTickets = totalTickets + p->ticket; // to find total no. of tickets
    }
    // printf("%d\n", totalTickets);
    long winner = random_at_most(totalTickets); // generating random variable fin range 0 to totalTickets
    for (p = proc; p < &proc[NPROC]; p++)       // iteration
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        ticketstillnow += p->ticket;
        if (ticketstillnow >= winner)
        {
          // Switch to chosen process.
          p->state = RUNNING;
          c->proc = p; // now process p will be run on cpu
  ...
  while (1)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        if (p->createtime < Startprocess->createtime)
        {
          if (Startprocess)
  ...
  while (1)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      niceness = 5; // niceness value

      if (p->runTime + p->sleepTime)
      {
        niceness = ((10 * p->sleepTime) / (p->sleepTime + p->runTime));
      }

      // int calc_prior = (p->priority - niceness + 5) < 100 ? (p->priority - niceness + 5) : 100;
      // int dynamic = 0 > calc_prior ? 0 : calc_prior;
      int mini = min(p->priority - niceness + 5, 100);
      int dynamic = max(0, mini);
      if (p->state == RUNNABLE)
      {
        if (NodeProc == 0)
        {
          highest_priority = dynamic;
          NodeProc = p;
          continue;
        }
        if (highest_priority > dynamic || ((highest_priority == dynamic) && ((NodeProc->num_runs > p->num_runs) || ((NodeProc->num_runs == p->num_runs) && (NodeProc->createtime > p->createtime)))))
        {
          if (NodeProc)
            release(&NodeProc->lock);
    ...
    // now nodeproc is the process which should be running
    if (NodeProc)
    {
      NodeProc->state = RUNNING;
      NodeProc->startTime = ticks;
      NodeProc->sleepTime = 0;
      NodeProc->runTime = 0;
      NodeProc->num_runs++;
      c->proc = NodeProc;
      swtch(&c->context, &NodeProc->context);
      // Process is done running for now.
      // It should have changed its p->state before coming back.
      c->proc = 0;
      release(&NodeProc->lock);
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
#if defined RR || defined FCFS || defined LBS
...
//       // but save old_ priority
//       old_ = p->priority;
//       p->priority = new;
//       release(&p->lock);

//       // if new priority is lowwer than original, yield
//       if (new < old_)
//         yield();
