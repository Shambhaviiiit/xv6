...
struct proc
{
  ...
  int pid;              // Process ID
  int ticket;           // for lottery based
  // wait_lock must be held when using this:
  struct proc *parent; // Parent process
  int is_sigalarm;     // for sigalarm and sigreturn
  int ticks;
  int now_ticks;

  uint64 handler;
  struct trapframe *trapframe_copy; // till here
...
struct PriorityQueue
{
  int front;
  int back;
  struct proc *queue[65];
};
extern struct PriorityQueue queuetable[5];
